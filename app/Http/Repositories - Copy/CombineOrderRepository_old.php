...
class CombineOrderRepository
{
  ...
  public function generateCutPlan(Request $request)
  {
    ...
    try {
      ...
      // find the biggest FPO
      $highest_fpo_id = '';
      $highest_fpo_qty_json = '';
      $sum = 0;
      $max_sum = 0;
      foreach ($fpos as $fpo) {
        ...
        if ($sum > $max_sum) {
          ...
          $highest_fpo_qty_json = $fpo->qty_json;
          $max_sum = $sum;
        }
      }

      // qty_json = ["S"=>100, "M"=>100, "L"=>100]
      // size_index is index of "S","M","L"
      foreach ($qty_json as $size_index => $value) {
        ...
        if (!(empty($cut_count_array[$size_index])) && !(empty($fpo_count_array[$size_index]))) {

          // if (sizeof($cut_count_array) >= sizeof($fpo_count_array)) {
          if (sizeof($cut_count_array[$size_index]) >= sizeof($fpo_count_array[$size_index])) {
            ...
            for ($i = 0; $i < sizeof($cut_count_array[$size_index]); $i++) {
              ...
              if (array_key_exists($i, $fpo_count_array[$size_index])) {
                ...
                // $concat_arr[$size_index][] = $fpo_count_array[$size_index][$last_item] . '^' . $cut_count_array[$size_index][$i];
                $concat_arr[$size_index][] = $highest_fpo_id . '^' . $cut_count_array[$size_index][$i];
              }
            }
          }
      ...
      foreach ($transform_array as $str => $value) {
        ...
        if (!($fpo_id == 0)) {
          $obj_fpofabric = FpoFabric::select('avg_consumption')->where('style_fabric_id', $style_fabric_id)->where('fpo_id', $fpo_id)->first();
