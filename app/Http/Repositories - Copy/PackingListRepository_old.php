...
class PackingListRepository
{
    ...
    public static function getFullPackingList($packing_list_id)
    {
        ...
        foreach ($packing_list_soc as $value) {
            ...
            $value['balance_qty_json'] = $soc->qty_json;
        ...
        try {
            ...
            if($revision_no == ""){
                ...
                $carton = DB::table('carton_packing_list')
                    ...
                    ->orderby('carton_packing_list.id')
                    ->get();
                $data = ['ID' => $packing_list_id,'soc'=>$soc,'carton'=>$carton, 'status'=>'Latest', 'revision_no'=>''];
            }else{
    ...
    public function updateBoxScanning($request){
        ...
        try {
            ...
            $currentDate = null;
            $currentDay = null;
            if(Carbon::now('GMT')->addMinutes(330)->format('H:i:s') < '09:15:00' && $shiftIDWanted == 2){
                $currentDate = Carbon::now('GMT')->subMinutes(555)->format('y-m-d');
                $currentDay = Carbon::now('GMT')->subMinutes(555)->format('l');
            }
            ...
            else if(Carbon::now('GMT')->addMinutes(330)->format('H:i:s') > '09:15:00' && $shiftIDWanted == 2){
                                    ...
                                    if($new_scan > 0){
                                        ...
                                        if($qty <= $update_ticket->original_quantity){
                                            DB::table('bundle_tickets')
                                                ->where('id', $pk_ticket_id)
                                                ->update(['scan_quantity' => $qty, 'daily_shift_team_id' => $dailyShiftTeamIdX, 'scan_date_time' => now(), 'packing_list_id' => $packing_list_id->packing_list_id, 'daily_scanning_slot_id' => $dailyScanSlotIdX, 'carton_id' => $request->box_id,'updated_by' => $request->user_id]);

                                            DB::insert('insert into bundle_ticket_secondaries (bundle_id, original_quantity, scan_quantity, packing_list_id,bundle_ticket_id,scan_date_time,daily_scanning_slot_id,daily_shift_team_id,carton_id,updated_by, created_by) values (?, ?, ?, ?,?, ?, ?, ?, ?,?,?)',
                                                [$rec->bundle_id, $rec->original_quantity, $new_scan, $packing_list_id->packing_list_id, $pk_ticket_id, now(), $dailyScanSlotIdX, $dailyShiftTeamIdX,$request->box_id, $request->user_id, $request->user_id]);
