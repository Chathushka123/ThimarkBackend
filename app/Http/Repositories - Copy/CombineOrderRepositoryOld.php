...
class CombineOrderRepository
{
  ...
  public function generateCutPlan(Request $request)
  {
    ...
    try {
      ...
      foreach($fpos as $rec){
        $qty = $rec->qty_json;
        //$qty=json_decode($rec->qty_json,true);
        $q = [];
        foreach($qty as $key=>$val){
          if(!isset($q[$key])){
            $q[$key] = $val+round($val*($rec->tolerance/100));
          }
        }
...
//      }
  ...
  public function getCombineOrderTotalQty($combine_order_id){
    ...
    foreach($fpos as $rec){
      $qty = $rec->qty_json;
      $tolerance = 0;
      if(!is_null($rec->tolerance) && $rec->tolerance > 0){
        $tolerance = $rec->tolerance;
      }
      foreach($qty as $key => $value){
        $sum_total_qty[$key] = $value + (array_key_exists($key, $sum_total_qty) ? $sum_total_qty[$key] : 0);
        $sum_total_qty_with_tolerance[$key] = ($value+round($value*($tolerance/100))) + (array_key_exists($key, $sum_total_qty_with_tolerance) ? $sum_total_qty_with_tolerance[$key] : 0);
      }
    }
