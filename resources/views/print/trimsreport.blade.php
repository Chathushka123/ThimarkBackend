...
<script type="text/php">
  ...
  <tr style = "font-size:12px;">
      <th style = "padding:5px; text-align:center; width: 30%; font-size:24px"> Job Card Report </th>
      <th style = "padding:5px; text-align:center; width: 10% "> Job Card No </th>
      <th style = "padding:5px; text-align:center; width: 10% "> {{$header[0]['job_card_no']."J"}} </th>
      <th style = "padding:5px; text-align:center; width: 20%;font-weight:50 "> <p>*{{$header[0]['job_card_no']."J"}}*</p> </th>
      <th style = "padding:5px; text-align:center; width: 10% "> SW Line </th>
      <th style = "padding:5px; text-align:center; width: 10% "> {{$header[0]['team']}} </th>
      <th style = "padding:5px; text-align:center; width: 10% "> {{$header[0]['team_id']}} </th>
  ...
  width: 100% ;">
    ...
    <tr style = "font-size:12px;">
      ...
      <th colspan="1" style = "padding:5px; text-align:left; width: 25% ">{{'VPO : '. $header[0]['vpo']}} </th>
    ...
    <tr style = "font-size:12px;">
      <th colspan="2" style = "padding:5px; text-align:left; width: 25% "> {{'Color : '. $header[0]['color']." (".$header[0]['color_code'].")"}} </th>
      <th colspan="2" style = "padding:5px; text-align:left; width: 25% ">{{'Country : '. $header[0]['destination']}}</th>
      
    </tr>
  </table>

  <table cellspacing="0" cellpadding="0" border="1" width="100%" style="border-color:#333; border-collapse: collapse;table-layout: fixed ;margin-top:10px;
  width: 100% ;">
    <tr style = "font-size:12px;">
    <th colspan="1" style = "padding:5px; text-align:left; width: 10% ">Fpo </th>
    <th colspan="1" style = "padding:5px; text-align:left; width: 10% ">Country </th>
    @foreach ($header[0]['sizewise'] as $key =>$val)
      <th colspan="1" style = "padding:5px; text-align:left; width: 10% ">{{$val['size']}} </th>
    @endforeach
    </tr>

    @foreach ($header[0]['fpo_brack_down'] as $key =>$val)
    <tr style = "font-size:12px;">
    <th colspan="1" style = "padding:5px; text-align:left; width: 10%; font-weight:50; ">{{$val['wfx_fpo_no']}} </th>
    <th colspan="1" style = "padding:5px; text-align:left; width: 10%; font-weight:50; ">{{$val['destination']}} </th>
      @foreach ($val['size'] as $k =>$v)
      <th colspan="1" style = "padding:5px; text-align:left; width: 10%; font-weight:50; ">{{$v['qty']}} </th>
      @endforeach
    </tr>
    @endforeach

    <tr style = "font-size:12px;">
    <th colspan="2" style = "padding:5px; text-align:left; width: 10% ">Total </th>
    @foreach ($header[0]['sizewise'] as $key =>$val)
      <th colspan="1" style = "padding:5px; text-align:left; width: 10% ">{{$val['qty']}} </th>
    @endforeach
    </tr>
  </table>
  ...
  <table cellspacing="0" cellpadding="0" border="1" width="100%" style="border-color:#333; border-collapse: collapse; margin-top:10px;">
    ...
    <?php
      $remark = $rec->special_remarks;
      if(is_null($remark)){
        $remark = $rec->special_remark;
      }
    ...
    <tr>
      ...
      <td style = "padding:5px; font-size:12px;">{{$rec->size}}</td>
	    ...
	    <td style = "padding:5px; font-size:12px;">{{$remark}}</td>
      <td style = "padding:5px; font-size:12px;">{{$rec->qty}}</td>
      <td style = "padding:5px; font-size:12px;">{{$rec->location_name}}</td>
      <td style = "padding:5px; font-size:12px;">{{$rec->combine_order_no .  "/".$rec->cut_no}}</td>
