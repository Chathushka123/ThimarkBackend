...
class WipDashboardController extends Controller
{
    ...
    public function floor(): JsonResponse
    {
        try {
            ...
            return $this->success('Floor dashboard retrieved successfully.', [
                ...
                'station_team_breakdown' => $this->stationTeamBreakdown($today),
                'hourly_trend' => $this->hourlyTrend($today),
                'team_hourly_trend' => $this->teamHourlyTrend($today),
            ]);
        } catch (Exception $e) {
    ...
    /**
     * Today's per-team scan/reject/rework totals, rates, and efficiency — a
     * running scoreboard for the floor screen (rendered as cards).
     */
    private function teamScoreboard($today)
    {
        ...
        $reworkResolved = BundleTicketReworkReturn::where('active', true)
            ...
            ->pluck('qty', 'daily_shift_team_id');
        // Standard minutes earned today: SUM(scan_qty * smv) per team, via
        // each scan's own work-order operation — a team can float across
        // more than one operation code in a day, so this is summed per
        ...
        return $teamIds->map(function ($id) use ($scanned, $rejected, $reworked, $reworkResolved, $earnedMinutes, $teams) {
    ...
    /**
     * Today's good/reject/rework counts per (operation, team) pair — feeds
     * the floor screen's bar chart, which needs both dimensions on the axis
     * since a team isn't tied to a single operation across the whole day.
     */
    private function stationTeamBreakdown($today)
    {
        $rows = function (string $table, string $qtyColumn) use ($today) {
        ...
        $good = $rows('bundle_ticket_secondaries', 'scan_qty');
    ...
    /**
     * The hour-bucket window for today's trend charts: the union of every
     * shift running today (falling back to the full day if none is on
     * record yet). With $clampToNow, the range stops at "now" (a chart that
     * grows through the day); without it, the range always runs the full
     * shift start-to-end so a scrollable grid can show every hour slot up
     * front, later ones simply empty until they happen.
     * Shared by hourlyTrend() and teamHourlyTrend() so both charts bucket
     * against the exact same hours.
     */
    private function todayHourRange($today, bool $clampToNow = true): array
    {
        $shiftsToday = DailyShift::where('shift_date', $today->toDateString())
            ->where('active', true)
            ->get();

        $rangeStart = $shiftsToday->isEmpty() ? $today->copy() : $shiftsToday->min('start_date_time');
        $rangeEndCandidate = $shiftsToday->isEmpty() ? $today->copy()->endOfDay() : $shiftsToday->max('end_date_time');
        $rangeEnd = ($clampToNow && now()->lt($rangeEndCandidate)) ? now() : $rangeEndCandidate;
    ...
    /**
     * Today's scanned qty per hour, split by ticket direction (IN vs OUT),
     * across the union of every shift running today — the floor screen's
     * "throughput over the day" line chart.
     */
    private function hourlyTrend($today)
    {
        [$rangeStart, $rangeEnd] = $this->todayHourRange($today);
     ...
     */
    private function teamHourlyTrend($today)
    {
        [$rangeStart, $rangeEnd] = $this->todayHourRange($today, false);
    ...
    /**
     * Today's main-model / model / batch WIP breakdown for a single team —
     * the drill-down behind clicking a team card on the floor screen.
     */
    public function teamBatchSummary(Request $request): JsonResponse
    {
        try {
            ...
            $today = now()->startOfDay();
            $groupBy = function (string $table, string $qtyColumn) use ($today, $teamId) {
                return DB::table("$table as t")
                    ->join('bundle_tickets as bt', 'bt.id', '=', 't.bundle_ticket_id')
                    ->join('bundles as b', 'b.id', '=', 'bt.bundle_id')
                    ->join('work_orders as wo', 'wo.id', '=', 'b.work_order_id')
            ...
            $rows = $keys->map(function ($key) use ($scanned, $rejected, $reworked, $reworkResolved) {
                $row = $scanned[$key] ?? $rejected[$key] ?? $reworked[$key];
                $scannedQty = (int) optional($scanned[$key] ?? null)->qty;
                $rejectedQty = (int) optional($rejected[$key] ?? null)->qty;
                $reworkQty = (int) optional($reworked[$key] ?? null)->qty;
                ...
                return [
                    ...
                    'rejected_qty' => $rejectedQty,
                    'rework_qty' => $reworkQty,
                    'wip_qty' => max(0, $scannedQty - $rejectedQty - $reworkOutstanding),
                    'reject_rate_pct' => $handled > 0 ? round($rejectedQty / $handled * 100, 1) : 0,
                    'rework_rate_pct' => $handled > 0 ? round($reworkQty / $handled * 100, 1) : 0,
                ];
            })->sortByDesc('scanned_qty')->values();
    ...
    public function management(Request $request): JsonResponse
    {
        try {
            ...
            return $this->success('Management dashboard retrieved successfully.', [
                ...
                'daily' => $daily,
                'batch_breakdown' => $this->batchBreakdown($from, $to),
                'team_scoreboard' => $this->teamScoreboardRange($from, $to),
                'station_team_breakdown' => $this->stationTeamBreakdownRange($from, $to),
                'team_daily_trend' => $this->teamDailyTrend($from, $to),
            ]);
        } catch (Exception $e) {
     ...
     */
    private function batchBreakdown($from, $to)
    {
        $scanned = $this->batchQtyByGroup('bundle_ticket_secondaries', 'scan_qty', $from, $to);
        $rejected = $this->batchQtyByGroup('bundle_ticket_rejects', 'reject_qty', $from, $to);
        $reworked = $this->batchQtyByGroup('bundle_ticket_reworks', 'rework_qty', $from, $to);
     ...
     */
    private function batchQtyByGroup(string $table, string $qtyColumn, $from, $to)
    {
        return DB::table("$table as t")
            ->join('bundle_tickets as bt', 'bt.id', '=', 't.bundle_ticket_id')
            ->join('bundles as b', 'b.id', '=', 'bt.bundle_id')
     ...
     */
    private function teamScoreboardRange(Carbon $from, Carbon $to)
    {
        $groupByTeam = function (string $table, string $qtyColumn) use ($from, $to) {
            return DB::table("$table as t")
                ->join('daily_shift_teams as dst', 'dst.id', '=', 't.daily_shift_team_id')
                ->where('t.active', true)
                ->where('t.created_at', '>=', $from)
        ...
        $scanned = $groupByTeam('bundle_ticket_secondaries', 'scan_qty');
        ...
        return $teamIds->map(function ($teamId) use ($scanned, $rejected, $reworked, $reworkResolved, $earnedMinutesByTeam, $availableMinutesByTeam, $teams) {
            $scannedQty = (int) ($scanned[$teamId] ?? 0);
     ...
     * daily_shift_team_id.
     */
    private function stationTeamBreakdownRange(Carbon $from, Carbon $to)
    {
        $rows = function (string $table, string $qtyColumn) use ($from, $to) {
            return DB::table("$table as t")
                ->join('bundle_tickets as bt', 'bt.id', '=', 't.bundle_ticket_id')
                ->join('work_order_operations as woo', 'woo.id', '=', 'bt.work_order_operation_id')
                ->join('routing_operation_masters as rom', 'rom.id', '=', 'woo.routing_operation_master_id')
     ...
     */
    private function teamDailyTrend(Carbon $from, Carbon $to)
    {
        $rows = BundleTicketSecondary::where('bundle_ticket_secondaries.active', true)
