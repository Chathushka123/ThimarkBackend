...
//        print_r($arr);
    ...
    public function bubbleSortNew($arr)
    {
//        print_r($arr);
        try {
            $n = sizeof($arr);
    ...
    public function scanByScanningSlotOPD($bundle_ticket_id, $daily_scanning_slot_id, $daily_shift_team_id,$packing_list_id,$scan_date_time, $user_id,$operation,$direction,$location)
    {
        try {
            ...
            if(strtoupper($operation) == "FG" && strtoupper($direction) == "IN"){
                ...
                BundleMovement::create(['bundle_id'=>$bundle_ticket_id,'location_id'=>$ranala_location->id,'scan_by'=>auth()->user()->id,'scan_date_time'=>now()]);
                ...
                BundleMovement::create(['bundle_id'=>$bundle_ticket_id,'location_id'=>$location,'scan_by'=>auth()->user()->id,'scan_date_time'=>now()]);
...
//            $nowDate = new \DateTime("now");
		   ...
		   $all_tickets = DB::select('select bundle_tickets.*, fpo_operations.operation, fpo_operations.operation_code , routing_operations.wfx_seq
                from bundle_tickets 
                inner join fpo_operations on fpo_operations.id = bundle_tickets.fpo_operation_id 
                inner join routing_operations on routing_operations.id = fpo_operations.routing_operation_id 
                where bundle_tickets.bundle_id = "'.$bundle_ticket->bundle_id.'"
                group by bundle_tickets.direction,fpo_operations.operation');
		   
		

            $xxx = $this->bubbleSortNew($all_tickets);
            $previousOp = $this->findPreviousOp($bundle_ticket_id);
            ...
            for ($i = 0; $i < $n; $i++){
                ...
                if($xxx[$i]->scan_quantity == null && $xxx[$i]->id === $bundle_ticket->id) {
                   // if ($xxx[$i]->direction == "OUT" && substr($xxx[$i]->fpo_operation->routing_operation->operation_code, 0, 2) == "SW") {
                   if ($xxx[$i]->direction == "OUT" && substr($xxx[$i]->operation_code, 0, 2) == "SW") {     throw new Exception("SW - OUT tickets are not allowed to scan.");
                    }
                }
            }
            ...
            foreach($all_tickets as $rec){
                ...
                //if($rec->fpo_operation->routing_operation->routing_id == $rout) {
                    if (is_null($rec->scan_quantity) && $op_seq > $seq) {
                       // $op = $rec->fpo_operation->routing_operation->operation_code . " - " . $rec->direction;
					   $op = $rec->operation_code . " - " . $rec->direction;
...
//                        throw new Exception("Not enough available quantity for size(" . $bundle_of_ticket->size . ") in the box to scan the quantity(" . $quantity . ") of bundle(" . $bundle_ticket->id . ").");
    ...
    public static function  validatePackinList($bundle_id,$box_id){
        ...
        try{
            ...
            BundleMovement::create(['bundle_id'=>$ticketId,'location_id'=>$ranala_location->id,'scan_by'=>auth()->user()->id,'scan_date_time'=>now()]);
...
//            print_r($receivedDateYMD);
		...
		//	$all_tickets = BundleTicket::select('bundle_tickets.*','fpo_operations.operation','fpo_operations.operation_code')
		 //  ->join('fpo_operations','fpo_operations.id', '=','bundle_tickets.fpo_operation_id' )
		 //  ->join('routing_operations','routing_operations.id', '=','fpo_operations.routing_operation_id' )
		   
		//   ->distinct('bundle_tickets.direction','fpo_operations.operation')
		//   ->where('bundle_id',$bundle_ticket->bundle_id)
...
// need to add a button to see the details of the box
                ...
                for ($i = 0; $i < $n; $i++) {
                    ...
                    if ($xxx[$i]->scan_quantity == null && $xxx[$i]->id === $bundle_ticket->id) {
                       // if ($xxx[$i]->direction == "OUT" && substr($xxx[$i]->fpo_operation->routing_operation->operation_code, 0, 2) == "SW") {
						if ($xxx[$i]->direction == "OUT" && substr($xxx[$i]->operation_code, 0, 2) == "SW") {
                            throw new Exception("SW - OUT tickets are not allowed to scan.");
                        }
                    }
                }
                ...
                foreach ($all_tickets as $rec) {
                  ...
                  $op = $rec->operation_code . " - " . $rec->direction;
                    if (substr($op, 0, 2) === "SW" && $rec->direction === "OUT") {
                       // $swOutOfBundle = $rec;
                       $swOutOfBundle = BundleTicket::find($rec->id);
...
//                        throw new Exception("Not enough available quantity for size(" . $bundle_of_ticket->size . ") in the box to scan the quantity(" . $quantity . ") of bundle(" . $bundle_ticket->id . ").");
    ...
    public function newPackingInScanningOPD2106($boxId, $ticketId, $quantity, $daily_scanning_slot_id, $daily_shift_team_id,$scan_date_time, $user_id, $rejected_qty, $rejected_reason,$operation,$direction){
        try{
            DB::beginTransaction();
...
//                        throw new Exception("Not enough available quantity for size(" . $bundle_of_ticket->size . ") in the box to scan the quantity(" . $quantity . ") of bundle(" . $bundle_ticket->id . ").");
    ...
    public function getvcode($bundle_ticket_id){
        try{
            $bundle_ticket = BundleTicket::find($bundle_ticket_id);
...
//                    ->get();
    ...
    //////////////////////////// NEW PACKING IN WINDOW  /////////////////////////

    public function getBoxDetailsCartonOPD($request){
        DB::statement("SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))");

        $details = PackingList::select('packing_list_soc.quantity_json as jsonOriginal','packing_list_details.qty_json as jsonDuplicate','packing_list_details.qty_json as jsonBoXOriginal', 'packing_list_soc.pack_ratio','packing_list_soc.packing_list_id', DB::raw('SUM(bundle_ticket_secondaries.scan_quantity) as scan_quantity'), 'fpos.wfx_fpo_no')
            ...
            ->leftJoin('bundle_ticket_secondaries', 'bundle_ticket_secondaries.carton_id', '=', 'packing_list_details.id')
            ->where('packing_list_details.id', $request->carton_id)
            ->groupBy('packing_list_soc.pack_ratio','fpos.wfx_fpo_no')
            ->get();

        $scanned = BundleTicketSecondary::select(DB::raw('SUM(bundle_ticket_secondaries.scan_quantity) as scan_quantity'), 'fpos.wfx_fpo_no', 'bundles.size')
            ->join('bundle_tickets', 'bundle_tickets.id', '=', 'bundle_ticket_secondaries.bundle_ticket_id')
            ->join('bundles', 'bundles.id', '=', 'bundle_tickets.bundle_id')
            ->join('fpo_operations', 'fpo_operations.id', '=', 'bundle_tickets.fpo_operation_id')
            ->join('fpos', 'fpos.id', '=', 'fpo_operations.fpo_id')
            ->where('bundle_ticket_secondaries.carton_id', $request->carton_id)
            ->where('fpo_operations.operation', "PK")
            ->where('bundle_tickets.direction', "IN")
            ->groupBy('fpos.wfx_fpo_no', 'bundles.size')
            ->get();

        foreach($details as $det){
            $fpoFound = false;
            $sizeFound = false;
            $jsonVal = json_decode($det->jsonDuplicate);
            foreach($jsonVal as $k => $v){
                $v = 0;
                foreach($scanned as $sc){
                    if($det->wfx_fpo_no == $sc->wfx_fpo_no && $k == $sc->size){
                        $fpoFound = true;
                        $sizeFound = true;
                        $v += $sc->scan_quantity;
            ...
            $det->jsonDuplicate = json_encode($jsonVal);
    ...
    public function ScanPKByCartonOPD($request){
        try{
            ...
            $pack_ration_sum_team = 1;
            $carton_id = $request->input('carton_id');
            $team_id = $request->input('team_id');
            $req_qty_json = null;

            if(isset($request->qty_data)){
                $req_qty_json = $request->qty_data;
            }

            $carton = PackingListDetail::where('id',$carton_id)->first();
            if(is_null($carton)){
                throw new GeneralException("InValid Carton ID");
            }
            ...
            $daily_shiftX = DailyShift::select('shift_details.shift_id')
            ...
            ->first();
             ...
             // if($request->preShift == "Yes"){
            if(false){
                $daily_shiftX = DailyShift::select('daily_shifts.*')
                    ->where('daily_shifts.start_date_time', '<=', $dateTime)
                    ->where('daily_shifts.end_date_time', '>=', $dateTime)
                    ->first();
                
                $extendVal = DB::table('parameters')->select('value')->where('name', 'scanning_extend')->first();

                $datetime1 = new \DateTime($dateTime);
                $datetime2 = new \DateTime($daily_shiftX->start_date_time);
        
                $interval = $datetime1->diff($datetime2);
                $hours = round($interval->s / 3600 + $interval->i / 60 + $interval->h + $interval->days * 24, 2);
                // $hours = $interval->h + $interval->days * 24;
        
                if($hours >= $extendVal->value){
                    throw new GeneralException("Extended time has passed. Not allowed to scan.");
                }
                $shift = DailyShift::select('daily_shifts.id' , 'daily_shifts.start_date_time', 'daily_shifts.end_date_time')
                    ->join('daily_shift_teams', 'daily_shift_teams.daily_shift_id', '=',  'daily_shifts.id')
                    ->where('daily_shifts.start_date_time', '<' , $daily_shiftX->start_date_time)
                    ->orderBy('daily_shifts.start_date_time', 'DESC')
                    ->first();
                
                $slot = DailyScanningSlot::select('daily_scanning_slots.id')
                    ->where('daily_scanning_slots.from_date_time','<=' ,$shift->end_date_time)
                    ->where('daily_scanning_slots.to_date_time','>=' ,$shift->end_date_time)
                    ->first();
                
                    if(is_null($shift)){
                        throw new GeneralException("Shift Not Found");
                    }

                $scan_date_time = $shift->end_date_time;
            }
            else{
                $shift = DailyShift::select('daily_shifts.id')
                    ->join('shift_details', 'shift_details.id', '=',  'daily_shifts.shift_detail_id')
                    ->join('shifts', 'shifts.id', '=',  'shift_details.shift_id')
                    ->where('daily_shifts.start_date_time','<=' ,$dateTime)
                    ->where('daily_shifts.end_date_time','>=' ,$dateTime)
                    ->where('shifts.id', 7)
                    ->first();     
                ...
                $slot = DailyScanningSlot::select('daily_scanning_slots.id')
                    ->where('daily_scanning_slots.from_date_time','<=' ,$dateTime)
                    ->where('daily_scanning_slots.to_date_time','>=' ,$dateTime)
                    ->where('daily_scanning_slots.daily_shift_id', $shift->id)
                    ->first();
            }
            ...
            if(is_null($dailyShiftTeam)){
                throw new GeneralException("Daily Shift Team Not Found");
            }


            if(is_null($slot)){
                throw new GeneralException("Daily Scanning Slot Not Found");
            }



            else{
                $pack_ratio_sum = PackingListSoc::where(['packing_list_id' => $carton->packing_list_id])->sum('pack_ratio');
                if ($pack_ratio_sum == 0) {
                    $pack_ratio_sum = 1;
                }
                ...
                $qty_json = $carton->qty_json;
                $originalQtys = $qty_json;
                foreach($qty_json as $key => $val){
                    if($val > 0){
                        $qty_json[$key] = $val * $pack_ratio_sum;
                        $originalQtys[$key] = $val * $pack_ratio_sum;

                        $scanQty =  BundleTicketSecondary::join('bundle_tickets','bundle_tickets.id' , '=' ,'bundle_ticket_secondaries.bundle_ticket_id' )
                        ->join('bundles','bundles.id' , '=' ,'bundle_tickets.bundle_id' )
                        ->join('fpo_operations','fpo_operations.id' , '=' ,'bundle_tickets.fpo_operation_id' )
                        ->where('fpo_operations.operation' , 'PK')
                        ->where('bundles.size' , $key)
                        ->where('bundle_tickets.direction','IN')
                        ->where('bundle_ticket_secondaries.carton_id',$carton_id)
                        ->sum('bundle_ticket_secondaries.scan_quantity');

                        if($pack_ratio_sum <= 1){
                            $qty_json[$key] -= $scanQty;
                        }


                        if(!is_null($req_qty_json)){
                            if(intval($req_qty_json[$key]) > intval($qty_json[$key])){
                                throw new GeneralException("Qty Exceed in Size ".$key."");
                            }else{
                                if($pack_ratio_sum > 1){
                                    $qty_json[$key] = $req_qty_json[$key] * $pack_ratio_sum;
                                    //$boxQty = $req_qty_json[$key] * $pack_ratio_sum;
                                }
                                else{
                                    $qty_json[$key] = $req_qty_json[$key];
                                }
                ...
                foreach($qty_json as $key => $val){
                    if($val > 0){
                        if($pack_ratio_sum > 1){
                            $boxQty = $qty_json[$key];
                            $originalBoxQty = $originalQtys[$key];
                            $lineInBundle = self :: getLineInBundleKitPack($team_id,$carton_id,$key,$val, $pack_ratio_sum, $boxQty, $originalBoxQty);
                        }
                        else{
                            $lineInBundle = self :: getLineInBundle($team_id,$carton_id,$key);
                        }
                        //return $lineInBundle;
                        $qty = $val;
                        
                        if(sizeof($lineInBundle) > 0){
                            for($i = 0; $i< sizeof($lineInBundle); $i++){
                                if($qty > 0){

                                    $data[$index]['bundle_id'] = $lineInBundle[$i]['bundle_id'];

                                    
                                                    //////////////////////////////////////////// PACKING LIST SOC QTY VALIDATION  ///////////
                                                    $bundle_soc= Soc::select('socs.id')
                                                    ->join('fpos','fpos.soc_id' , '=' ,'socs.id' )
                                                    ->join('job_cards','job_cards.fpo_id' , '=' ,'fpos.id' )
                                                    ->join('job_card_bundles','job_card_bundles.job_card_id' , '=' ,'job_cards.id' )
                                                    ->where('job_card_bundles.bundle_id' , $lineInBundle[$i]['bundle_id'])
                                                    ->first();
                                                       // print_r($carton->packing_list_id);
                                                    $packing_soc = PackingListSoc::select('packing_list_soc.quantity_json')   
                                                    ->where('packing_list_soc.soc_id',$bundle_soc->id)
                                                    ->where('packing_list_id',$carton->packing_list_id)
                                                    ->first();

                                                    $packing_soc_qty_json = $packing_soc->quantity_json;
                
                                                    $packing_soc_qty = $packing_soc_qty_json[$key];
                
                                                    
                                                    $sum_of_scan_qty = DB::select('select sum(bts.scan_quantity) as ScanQty from bundle_ticket_secondaries bts 
                                                    join bundle_tickets bt on bt.id = bts.bundle_ticket_id 
                                                    join fpo_operations fo on fo.id = bt.fpo_operation_id 
                                                    join bundles b on b.id = bt.bundle_id 
                                                    join fpos f on f.id = fo.fpo_id
                                                    where bts.packing_list_id = "'.$carton->packing_list_id.'" and f.soc_id = "'.$bundle_soc->id.'" and b.size = "'.$key.'" and fo.operation = "PK" and bt.direction = "IN"');
                                                    
                                                   
                                                    if(sizeof( $sum_of_scan_qty) > 0 && ($sum_of_scan_qty[0]->ScanQty) > 0){
                                                       // print_r($sum_of_scan_qty[0]->ScanQty);
                                                      //  print_r("P");
                                                        $packing_soc_qty -= $sum_of_scan_qty[0]->ScanQty;
                                                    }
                                                   // print_r("A");
                                                 //  print_r(intval($lineInBundle[$i]['qty'])."|".intval($packing_soc_qty)."/");
                                                    if(intval($lineInBundle[$i]['qty']) > intval($packing_soc_qty)){
                                                        $lineInBundle[$i]['qty'] = $packing_soc_qty; 
                                                       // print_r($lineInBundle[$i]['qty']."///");
                                                    }
                                                    
                                            /////////////////////////////////////////////////////////////////

                                    if(intval($lineInBundle[$i]['qty']) >= $qty){
                                        $data[$index]['qty'] = $qty;
                                        $qty = 0;
                                        $this->updateBundleDetails($data,$index,$scan_date_time,$dailyShiftTeam, $slot,$carton_id,$carton);
                                        $index++;
                                        break;
                                    }else if(intval($lineInBundle[$i]['qty']) > 0){
                                        $data[$index]['qty'] = $lineInBundle[$i]['qty'];
                                        $this->updateBundleDetails($data,$index,$scan_date_time,$dailyShiftTeam, $slot,$carton_id,$carton);
                                        $qty -= intval($lineInBundle[$i]['qty']);
                                        $index++;
                                    }
                            ...
                            if($qty > 0 && $pack_ratio_sum <= 1){
                                throw new GeneralException("insufficient Line In Qty For Size ".$key."");
                            }
    ...
    public static function getLineInBundleKitPack($team_id,$carton_id,$size,$balanceQty, $pack_ratio_sum, $boxQty, $originalBoxQty){

        $carton = PackingListDetail::where('id',$carton_id)->first();
        $pk_list_soc = PackingListSoc::where('packing_list_id', $carton->packing_list_id)->get();

		$pk_list_soc_ids = PackingListSoc::where('packing_list_id', $carton->packing_list_id)->pluck('soc_id')->toArray();
        $job_cards = JobCard::join('packing_lists','packing_lists.id','=','job_cards.packing_list_no')
            ->join('packing_list_details','packing_list_details.packing_list_id','=','packing_lists.id')
            ->where('packing_list_details.id',$carton_id)
            ->pluck('job_cards.id')->toArray();

        DB::statement("SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))");

        $job_or_soc = DB::select("select value from parameters p where p.name = 'pk_in_scan_job_or_soc_validation'");

        if ($job_or_soc[0]->value == 'job') {
            $bundle_tickets = PackingList::select('bundle_tickets.id', 'bundle_tickets.original_quantity', 'bundle_tickets.scan_quantity', 'socs.id as soc_id', 'bundles.size','bundles.id as bundle_id')
            ->join('job_cards', 'job_cards.packing_list_no', '=', 'packing_lists.id')
            ->join('fpos', 'fpos.id', '=', 'job_cards.fpo_id')
            ->join('socs', 'socs.id', '=', 'fpos.soc_id')
            ->join('job_card_bundles', 'job_card_bundles.job_card_id', '=', 'job_cards.id')
            ...
            ->join('fpo_operations', 'fpo_operations.id', '=', 'bundle_tickets.fpo_operation_id')
            ->whereIn('job_cards.id', $job_cards)
            ->where('fpo_operations.operation', 'SW')
            ->where('bundle_tickets.direction', 'IN')
            ->where('bundles.size', $size)
            ->where('bundle_tickets.scan_quantity', '>', 0)
            ->where('daily_shift_teams.team_id', $team_id)
            ->orderBy('bundle_tickets.scan_date_time', 'ASC')
            ->get();
        }
        else{
			...
			$bundle_tickets = BundleTicketSecondary::select('bundle_tickets.id', 'bundle_tickets.original_quantity', 'bundle_tickets.scan_quantity', 'socs.id as soc_id', 'bundles.size','bundles.id as bundle_id')
                ->join('bundle_tickets', 'bundle_tickets.id', '=', 'bundle_ticket_secondaries.bundle_ticket_id')
                ->join('bundles', 'bundles.id', '=', 'bundle_tickets.bundle_id')
                ->join('job_card_bundles', 'job_card_bundles.bundle_id', '=', 'bundle_tickets.bundle_id')
                ->join('job_cards', 'job_cards.id', '=', 'job_card_bundles.job_card_id')
                ->join('fpos', 'fpos.id', '=', 'job_cards.fpo_id')
				->join('socs', 'socs.id', '=', 'fpos.soc_id')
                ->join('fpo_operations', 'fpo_operations.id', '=', 'bundle_tickets.fpo_operation_id')
                ->join('daily_shift_teams', 'daily_shift_teams.id', '=', 'bundle_tickets.daily_shift_team_id')
                ->whereIn('fpos.soc_id', $pk_list_soc_ids)
                ->where('fpo_operations.operation', 'SW')
                ->where('bundle_tickets.direction', 'IN')
                ->where('daily_shift_teams.team_id', $team_id)
                ->groupBy('bundle_tickets.id')
                ->where('bundles.size', $size)
                ->orderBy('bundle_tickets.scan_date_time', 'ASC')
                ->get();
        }
...
//return $bundle_tickets;
        ...
        $index = 0;
        foreach($bundle_tickets as $rec){
            $swOutBundle=BundleTicketSecondary::select( DB::raw('SUM(bundle_ticket_secondaries.scan_quantity) as scan_quantity'))
                ->join('bundle_tickets','bundle_tickets.id' , '=' ,'bundle_ticket_secondaries.bundle_ticket_id' )
                ->join('bundles','bundles.id' , '=' ,'bundle_tickets.bundle_id' )
                ->join('fpo_operations','fpo_operations.id' , '=' ,'bundle_tickets.fpo_operation_id')
            ...
            $pk_in_ticket=BundleTicket::select('bundle_tickets.id')
                ->join('fpo_operations','fpo_operations.id' , '=' ,'bundle_tickets.fpo_operation_id')
            ...
            $reject = QcReject::select( DB::raw('SUM(quantity) as quantity'))
                ->groupByRaw('bundle_ticket_id')
                ->where('bundle_ticket_id', $pk_in_ticket->id)
                ->first();


            if(!is_null($swOutBundle)){

                if(intval($rec->scan_quantity) > intval($swOutBundle->scan_quantity)){
                    $data_array[$index]['bundle_id'] = $rec->bundle_id;
                    $data_array[$index]['soc_id'] = $rec->soc_id;
                    $data_array[$index]['qty'] = intval($rec->scan_quantity) - intval($swOutBundle->scan_quantity);

                    if(!is_null($reject)){

                        $data_array[$index]['qty'] -= $reject->quantity;
                    }
                ...
                $data_array[$index]['bundle_id'] = $rec->bundle_id;
                $data_array[$index]['soc_id'] = $rec->soc_id;
                $data_array[$index]['qty'] = intval($rec->scan_quantity);
        ...
        foreach($pk_list_soc as $plc){

            $protionQtyForSOC = $boxQty * $plc->pack_ratio / $pack_ratio_sum;
            $originalBoxQtyX = $originalBoxQty  * $plc->pack_ratio / $pack_ratio_sum;

            $scannedForSOCBox = BundleTicketSecondary::select(DB::raw('SUM(bundle_ticket_secondaries.scan_quantity) as scan_quantity'), 'fpos.wfx_fpo_no')
            ->join('bundle_tickets', 'bundle_tickets.id', '=', 'bundle_ticket_secondaries.bundle_ticket_id')
            ->join('fpo_operations', 'fpo_operations.id', '=', 'bundle_tickets.fpo_operation_id')
            ->join('fpos', 'fpos.id', '=', 'fpo_operations.fpo_id')
            ->join('bundles', 'bundles.id', '=', 'bundle_tickets.bundle_id')
            ->where('bundle_ticket_secondaries.carton_id', $carton_id)
            ->where('fpos.soc_id', $plc['soc_id'])
            ->where('bundles.size', $size)
            ->groupBy('fpos.wfx_fpo_no')
            ->get();

            $totalScannedSOCBox = 0;
            
            foreach($scannedForSOCBox as $sc){
                $totalScannedSOCBox += $sc->scan_quantity;
            }


            $totalQtyFilling = 0;

            // if($totalScannedSOCBox < $originalBoxQtyX){
                foreach($data_array as $da){

                    if($da['soc_id'] == $plc['soc_id']){

                        if($da['qty'] > $protionQtyForSOC){
                            $da['qty'] = $protionQtyForSOC;
                        }

                        if($da['qty'] + $totalScannedSOCBox > $originalBoxQtyX){
                            $da['qty'] = $originalBoxQtyX - $totalScannedSOCBox;
                        }

                        if($da['qty'] > 0 && $da['qty'] + $totalScannedSOCBox <= $originalBoxQtyX){

                            $da['qty'] =  floor($da['qty'] / $plc->pack_ratio) * $plc->pack_ratio;

                            if($da['qty'] > 0){
                            array_push($selectedValues, $da);
                            
                                $totalQtyFilling += $da['qty'];
                                $totalScannedSOCBox += $da['qty'];
                                $protionQtyForSOC -= $da['qty'];
                            }
                        }
                    }
                }
    ...
    public static function getLineInBundle($team_id,$carton_id,$size){
        $packing_soc = PackingListSoc::join('packing_list_details','packing_list_details.packing_list_id','=','packing_list_soc.packing_list_id')
        ->where('packing_list_details.id',$carton_id)
        ->pluck('packing_list_soc.soc_id')->toArray();
        ...
        $job_or_soc = DB::select("select value from parameters p where p.name = 'pk_in_scan_job_or_soc_validation'");
        $commonValueRow = DB::select("select common_shift_value from teams t where t.id = ".$team_id."");
        $commonValue = $commonValueRow[0]->common_shift_value;
        $csv = DB::select("select value from parameters p where p.name = 'CommonShiftValue'");

        if ($job_or_soc[0]->value == 'job') {
            if($csv[0]->value == 1 && !is_null($commonValue)){
                $swInBundle = BundleTicketSecondary::select(DB::raw('SUM(bundle_ticket_secondaries.scan_quantity) as scan_quantity'), 'bundle_tickets.bundle_id', 'bundle_tickets.id', 'fpo_operations.operation', 'bundle_tickets.direction')
                ->join('bundle_tickets', 'bundle_tickets.id', '=', 'bundle_ticket_secondaries.bundle_ticket_id')
                ->join('bundles', 'bundles.id', '=', 'bundle_tickets.bundle_id')
                ->join('job_card_bundles', 'job_card_bundles.bundle_id', '=', 'bundle_tickets.bundle_id')
                ->join('job_cards', 'job_cards.id', '=', 'job_card_bundles.job_card_id')
                ...
                ->where('fpo_operations.operation', 'SW')
                ->where('bundle_tickets.direction', 'IN')
                ->where('teams.common_shift_value', $commonValue)
                ->groupBy('bundle_tickets.id')
                ->where('bundles.size', $size)
                ->orderBy('bundle_tickets.scan_date_time', 'ASC')
                ->get();
            ...
            if($csv[0]->value == 1 && !is_null($commonValue)){
                ...
                ->join('daily_shift_teams', 'daily_shift_teams.id', '=', 'bundle_tickets.daily_shift_team_id')
                ->join('teams', 'teams.id', '=', 'daily_shift_teams.team_id')
                ->whereIn('fpos.soc_id', $packing_soc)
                ->where('fpo_operations.operation', 'SW')
                ->where('bundle_tickets.direction', 'IN')
               // ->where('daily_shift_teams.team_id', $team_id)
                ->where('teams.common_shift_value', $commonValue)
    ...
    private  function updateBundleDetails($data,$j,$scan_date_time,$dailyShiftTeam, $slot,$carton_id,$carton){
        $packing_location = DB::table('locations')->select('id')
        ->where('location_name','=','Packing')
        ->first();

        $bundle = Bundle::find($data[$j]['bundle_id']);
        $bundle->update(['location_id'=>$packing_location->id]);

        $sw = BundleTicket::select('bundle_tickets.id','scan_quantity')
        ->join('fpo_operations','fpo_operations.id','=','bundle_tickets.fpo_operation_id')
        ->where('bundle_tickets.bundle_id',$data[$j]['bundle_id'])
        ->where('fpo_operations.operation','SW')
        ->where('bundle_tickets.direction','OUT')
        ->first();


        $pk = BundleTicket::select('bundle_tickets.id')
        ->join('fpo_operations','fpo_operations.id','=','bundle_tickets.fpo_operation_id')
        ->where('bundle_tickets.bundle_id',$data[$j]['bundle_id'])
        ->where('fpo_operations.operation','PK')
        ->where('bundle_tickets.direction','IN')
        ...
        $sw_out = BundleTicket::find($sw->id);
        $pk_in = BundleTicket::find($pk->id);
        

        //////////////////// SW OUT TICKET   ///////////////
        $swOutQty = (!is_null($sw_out->scan_quantity)) ? intval($sw_out->scan_quantity) : 0;
        $pkInQty = (!is_null($pk_in->scan_quantity)) ? intval($pk_in->scan_quantity) : 0;

        $sw_out->update([
            'scan_quantity' => $swOutQty+$data[$j]['qty'],
            'scan_date_time' => $scan_date_time,
            'daily_shift_team_id' => $dailyShiftTeam->id,
        ...
        BundleTicketSecondary::insert([
            'bundle_ticket_id' => $sw_out->id,
            'daily_scanning_slot_id' => $slot->id,
            'daily_shift_team_id' => $dailyShiftTeam->id,
            'packing_list_id' => null,
            'bundle_id' => $data[$j]['bundle_id'],
            'original_quantity' => $sw_out->original_quantity,
            "scan_quantity" => $data[$j]['qty'],
            'carton_id' =>null,
            "scan_date_time" => $scan_date_time,
        ...
        $pk_in->update([
            'scan_quantity' => $pkInQty+$data[$j]['qty'],
            'scan_date_time' => $scan_date_time,
            'daily_shift_team_id' => $dailyShiftTeam->id,
            'daily_scanning_slot_id' => $slot->id,
            'updated_by' => Auth::user()->id,
        ...
        BundleTicketSecondary::insert([
            'bundle_ticket_id' => $pk_in->id,
            'daily_scanning_slot_id' => $slot->id,
            'daily_shift_team_id' => $dailyShiftTeam->id,
            'packing_list_id' => $carton->packing_list_id,
            'bundle_id' => $data[$j]['bundle_id'],
            'original_quantity' => $sw_out->original_quantity,
            "scan_quantity" => $data[$j]['qty'],
            'carton_id' => $carton_id,
        ...
        BundleMovement::insert(['bundle_id'=>$data[$j]['bundle_id'],'location_id'=>$packing_location->id,'scan_by'=>auth()->user()->id,'scan_date_time'=>now()]);
        $this->_handleJobCardStatus($pk_in);
