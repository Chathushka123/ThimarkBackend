...
class FppoRepository
{
  ...
  public function createPendingBundleTicket(){
	              /////////////////////////////  Create Bundle Ticket   ///////////////////////////////////

   
			
			$job_card_bundles = DB::select('select b.id as bundle_id,b.quantity as original_quantity,fo.id as fpo_operation_id,fo.operation from bundles b
				join fpo_cut_plans fcp on fcp.fppo_id = b.fppo_id 
				join fpo_operations fo on fo.fpo_id = fcp.fpo_id 
				Left join bundle_tickets bt on b.id = bt.bundle_id 
				where bt.bundle_id is null');
				
			

            
            foreach($job_card_bundles as $rec){
				$bundle_ticket = BundleTicket::where('bundle_id','$rec->id')->first();
